...
function inicializarEventos() {
  ...
  $(document)
    .off("click", ".btn-activar")
    .on("click", ".btn-activar", function (e) {
      e.preventDefault();
      e.stopPropagation();
  ...
  $(document)
    .off("click", ".btn-ejecutar")
    .on("click", ".btn-ejecutar", function (e) {
      e.preventDefault();
      e.stopPropagation();
      const id = $(this).data("id");
  ...
  $(document)
    .off("click", ".btn-posponer")
    .on("click", ".btn-posponer", function (e) {
      e.preventDefault();
      e.stopPropagation();
      const id = $(this).data("id");
  ...
  $(document)
    ...
    .on("click", ".btn-eliminar", function (e) {
      e.preventDefault();
      e.stopPropagation();
      const id = $(this).data("id");
  ...
  $(document)
    .off("click", ".concepto-card")
    .on("click", ".concepto-card", function (e) {
      e.preventDefault();
...
function crearCardProgramacion(p) {
  ...
  const proxima = p.proximaEjecucion
    ? formatearFecha(p.proximaEjecucion)
    : "Sin definir";
  const descripcion =
    p.descripcion || '<em class="text-muted">Sin descripción</em>';
...
function actualizarBadgePendientes() {
  ...
  const pendientes = programaciones.filter(
    (p) => p.activo && p.proximaEjecucion && p.proximaEjecucion <= hoy
  ).length;
...
function establecerFechaMinima() {
  const hoy = new Date().toISOString().split("T")[0];
  $("#crearFechaInicio, #editarFechaInicio, #editarProximaEjecucion").attr(
    "min",
    hoy
  );
}
