document.addEventListener("DOMContentLoaded", function () {
  const API_BASE = "/api/movimientos/ingresos";
  ...
  const modalForm = new bootstrap.Modal(
    document.getElementById("modalFormIngreso")
  );
  ...
  // Event Listeners
  formIngreso.addEventListener("submit", handleFormSubmit);
  document
  ...
  function cargarDatosMensuales() {
    ...
    const pConceptos = fetch(`${API_CONCEPTOS}/tipo/INGRESO`, {
      credentials: "include",
    }).then((res) => {
                ...
                <div class="card h-100 border-0 shadow-sm hover-card" style="cursor: pointer; transition: transform 0.2s;">
                    <div class="card-body">
                        <div class="d-flex align-items-center mb-3">
                            <div class="rounded bg-success text-white d-flex align-items-center justify-content-center me-3" style="width: 50px; height: 50px;">
                        ...
                        <h3 class="fw-bold text-success mb-1">$${formatoMoneda(
                          c.totalAcumulado
                        )}</h3>
  ...
  function abrirDetalleConcepto(concepto) {
    ...
    if (concepto.movimientos && concepto.movimientos.length > 0) {
      concepto.movimientos.forEach((ingreso) => {
        const tr = document.createElement("tr");
        const fecha = new Date(ingreso.fechaRegistro).toLocaleDateString();
        tr.innerHTML = `
                      <td>${fecha}</td>
                      <td>${ingreso.descripcion || "-"}</td>
                      <td class="text-end fw-bold text-success">$${formatoMoneda(
                        ingreso.monto
                      )}</td>
                      <td class="text-end">
  ...
  window.abrirModalSeleccionConcepto = function () {
    ...
    fetch(`${API_CONCEPTOS}/tipo/INGRESO`, { credentials: "include" })
      .then((res) => res.json())
      .then((conceptos) => {
        listaConceptosSeleccion.innerHTML = "";
        conceptos.forEach((c) => {
  ...
  function abrirModalEditar(ingreso) {
    document.getElementById("modalFormTitle").textContent = "Editar Ingreso";
    document.getElementById("ingresoId").value = ingreso.id;
    document.getElementById("montoInput").value = ingreso.monto;
    document.getElementById("descripcionInput").value = ingreso.descripcion;
    ...
    fetch(`${API_CONCEPTOS}/${ingreso.conceptoId}`, { credentials: "include" })
      .then((res) => res.json())
      .then((concepto) => {
        document.getElementById("conceptoId").value = concepto.id;
        document.getElementById("conceptoInput").value = concepto.nombre;
  ...
  function handleFormSubmit(e) {
    ...
    fetch(url, {
      ...
      .then((response) => {
        if (!response.ok) throw new Error("Error guardando ingreso");
